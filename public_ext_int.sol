//take in consideration that public is easily accessed by other contracts
//private functions only accesible by other functons within the contract. private is visible as it's written in clear text on the blockchain. private only prevents other contracts to modify the information (but can be accessed by getter functions)
//internal, same as private but including being accessible from the derived contracts (hum... interesting for the crypto state real state functions)
//external is public but only accessible through transactions. NOT accessible from within the contract. external functions are cheaper than public.

//contracts can deploy another contracts
